...
$(document).ready(function () {
    $("#createEstimateButton").click(function () {
        ...
        if (estimateNameInput !== "" && estimateStartDate !== "" && estimateEndDate !== "" && contract !== "" && services !== "" && rateCard !== "") {
            ...
            // // fetch weight range data and set in the weight range label  
            // const weightRangeLabel = document.getElementById('weightRangeLabel');
            // fetch(`https://cleanstation.backendless.app/api/services/Estimate/ContractIDToServices?ID=${FulfillmentContratID}`)
            // .then(response => response.json())
            // .then(data => {
            //     ContractIDToServices = data; // Store the API response in the global variable
            //     console.log(ContractIDToServices); // Optional: Log the response to the console
            //     if (ContractIDToServices && ContractIDToServices.weightRange) {
            //     weightRangeLabel.textContent = `Weight Range: ${apiResponse.weightRange}`;
            //     } else {
            //     weightRangeLabel.textContent = 'Weight Range: Not available';
            //     }
            // })
            // .catch(error => {
            ...
            fetch(`https://cleanstation.backendless.app/api/services/Estimate/ContractIDToServices?ID=${FulfillmentContratID}`)
              ...
              .then((data) => {
                ...
                ContractIDToServices = data.Carrier_Services;

                // Find the selected service in the array
                const selectedServiceData = ContractIDToServices.find((Carrier_Services) => Carrier_Services.Weight_Range === ServiceID);

                if (selectedServiceData && selectedServiceData.Weight_Range) {
                  weightRangeLabel.textContent = `Weight Range: ${selectedServiceData.Weight_Range}`;
                } else {
                  weightRangeLabel.textContent = "Weight Range: Not available";
                }
              })
  ...
  fetch(updateServiceUrl)
    ...
    .then((updateServiceData) => {
      ...
      for (var entry of services) {
        ...
        var serviceWeightRange = entry.Weight_Range || "N/A";
        updateServiceList.options[updateServiceList.options.length] = new Option(
          serviceName,
          entry.objectId
        );
...
updateESTweightrangeinput.addEventListener("input", updateWeightRange);
updateESTweightrangeSlider.addEventListener("input", updateWeightRange);
