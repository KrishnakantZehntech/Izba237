...
function updateDataResult() {
    ...
    if (!isNaN(weightInput) && !isNaN(zoneInput)) {
        // Find the matching zone in the API response based on input values
        const matchingZone = ServiceIDToZoneData.find((zone) => {
            return zone.Weight_OZ >= weightInput && zone.Zone_1 === zoneInput;
        });

        // Update #dataResult with the matching zone price
        if (matchingZone) {
            dataResult.textContent = `Price for Zone ${zoneInput} and weight ${weightInput} oz: ${matchingZone.Price}`;
        } else {
            dataResult.textContent = "No matching zone found.";
        }
    } else {
        dataResult.textContent = "Invalid input values.";
    }
}
    ...
    let estimateData =
    {
        ...
        Average_Cost: parseFloat(matchingZone) ?? 0,
