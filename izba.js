...
$("#goToReciving").click(function () {
  ...
  if (brandNameInput !== "" && startDate !== "" && endDate !== "" && centers !== "") {
    // Check if brandId is defined (not null or undefined) and if it's a duplicate
    if (brandId) {
      alert("Duplicate brand entry");
      return;
    }
...
$(document).ready(function () {
  ...
  $("#MonthlyManagementFeeInput, #MonthlyMinimumFeeInput").on(
    ...
    function () {
      if (
        $("#MonthlyManagementFeeInput").val() &&
        $("#MonthlyMinimumFeeInput").val()
      ) {
        errorMisc1.style.display = "none";
      } else {
  ...
  $("#goToEstimates").click(function () {
    ...
    console.log("parasPostID", brandId);
    if (
