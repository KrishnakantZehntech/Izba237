...
$(document).ready(function () {
  ...
  $("#goToEstimates").click(function () {
    ...
    if (CheckManagementInputError() && CheckMiscInputError()) {
      ...
      fetch(
        "https://cleanstation.backendless.app/api/services/Estimate/LogOutCreateContract",
        {
          method: "POST",
...
$(document).ready(function () {
  ...
  $("#gotoEstimateDetail").click(function () {
    ...
    let estimatorDetails = {
      ...
      ReceiptPallet:
        PalletsReceivedInput !== "" ? parseFloat(PalletsReceivedInput) : 0,
...
function SetEstimateDetail(id) {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      let TotalSpents =
        data.TotalCost / data.TotalOrders == 0 ? 1 : data.TotalOrders;
      TotalSpents =
        data.TotalOrders != 0
          ? "$ " + data.TotalCost / data.TotalOrders
          : "No order found!";
      ...
      document.getElementById("TotalPickFeesCalc").textContent =
        "$ " + data.TotalPickCharge;
      document.getElementById("TotalStorage").textContent =
        "$ " + data.TotalStorageCharge;
      document.getElementById("TotalkReceipt").textContent =
        "$ " + data.TotalReceiptCharge;
      document.getElementById("TotalMisclabor").textContent =
...
$("#addFccenter").click(function () {
  ...
  fetch(
    ...
    .then((data) => {
      document.getElementById("FC-Form").style.display = "flex";
  ...
  fetch(
    ...
    .then((Data) => {
      ...
      document.getElementById("bname").value =
        Data.Fulfillment_contract.Contract_name;
      // Set the current values in the input fields
      document.getElementById("updateStartDate").value = formatDate(
        Data.Fulfillment_contract.Start_Date
      );
      document.getElementById("updateEndDate").value = formatDate(
        Data.Fulfillment_contract.End_Date
      );
...
$("#FinalupdateButton").click(function () {
  const UpdateEstimateData = {
    Fulfillment_Contract: {
      ...
      Storage_Per_Bin:
        document.getElementById("updateStoragePerBin").value === ""
          ? 0
          : parseFloat(document.getElementById("updateStoragePerBin").value),
      Storage_Per_Shelf:
        document.getElementById("updateStoragePerShelf").value === ""
          ? 0
          : parseFloat(document.getElementById("updateStoragePerShelf").value),
      Storage_Per_Pallet:
        document.getElementById("updateStoragePerPallet").value === ""
          ? 0
      ...
      Receipt_Per_Pallet_Fee:
        document.getElementById("updateReceiptPerPallet").value === ""
          ? 0
          : parseFloat(document.getElementById("updateReceiptPerPallet").value),
    ...
    Estimate: {
      objectId: EstimateId,
      ReceiptPallet:
        +document.getElementById("updatePalletReceived").value || 0,
      ...
      ExtraPicks:
        +document.getElementById("updateNumberAdditionlPick").value || 0,
  ...
  fetch(
    `https://cleanstation.backendless.app/api/services/Estimate/LogOutUpdate`,
    {
      method: "PUT",
      headers: {
...
function updateTotalDays() {
  ...
  if (updateStartDateValue && updateEndDateValue) {
    ...
    const oneDay = 24 * 60 * 60 * 1000;
    // Calculate the difference in days
    const updateDiffDays = Math.round(
      Math.abs((updateStartDate - updateEndDate) / oneDay)
    );
    // Display the result
    updateTotalDaysElement.textContent = updateDiffDays + " days";

    // Set the maximum selectable date in the updateStartDate picker
    const maxUpdateStartDate = new Date(updateEndDate);
    maxUpdateStartDate.setDate(maxUpdateStartDate.getDate() - 1); // Decrement by 1 day to disable the selected end date
    updateStartDateInput.max = maxUpdateStartDate.toISOString().slice(0, 10);
  } else {
    // Handle the case when either input is empty
