      ...
      for (const prop in obj) {
        if (typeof obj[prop] === 'object' && obj[prop] !== null) {
          if (seen.has(obj[prop])) {
            // Handle circular reference, e.g., replace with null or a placeholder.
            obj[prop] = null;
          } else {
            seen.add(obj[prop]);
            removeCircularReferences(obj[prop], seen);
          }
        }
      }
    }
    
    // Call the function to remove circular references
    removeCircularReferences(requestBody);
    
    // Now, you can safely use JSON.stringify
