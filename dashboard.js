...
async function productsAlldata(userObjID) {
  ...
  const table = $("#invoiceCSV-table-body");
  
  // Add a class to hide the table while data is being loaded
  table.addClass("hidden");
  
  // Display a loading message or indicator
  table.html('<tr><td colspan="19">Loading...</td></tr>');
  ...
  if (data.length === 0) {
    table.html('<tr><td colspan="19">No data found</td></tr>');
  } else {
    table.html(
      data
        .map((item) => {
  ...
  // Remove the class to show the table after loading the new data
...
async function populateDropdown() {
  ...
  if (productData.length > 0) {
    ...
    productData.forEach((product, index) => {
      ...
      if (index === 0) {
        // Select the most recently created item
